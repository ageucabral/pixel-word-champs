-- Corrigir função get_monthly_invite_stats para usar data de uso do convite ao invés de criação
...
BEGIN
  ...
  -- CORREÇÃO: Usar i.used_at ao invés de i.created_at
  WITH monthly_data AS (
    SELECT 
      ...
      -- NOVO CRITÉRIO: Ordenar EXCLUSIVAMENTE por pontos, com created_at como desempate
      ROW_NUMBER() OVER (
        ORDER BY 
  ...
  WITH top_3_data AS (
    SELECT 
      ...
      COUNT(CASE WHEN invited_p.games_played > 0 THEN 1 END) as active_invites_count,
      (COUNT(i.id) * 10 + COUNT(CASE WHEN invited_p.games_played > 0 THEN 1 END) * 40) as invite_points,
      -- MESMO CRITÉRIO: Ordenar EXCLUSIVAMENTE por pontos
      ROW_NUMBER() OVER (
        ORDER BY 
          (COUNT(i.id) * 10 + COUNT(CASE WHEN invited_p.games_played > 0 THEN 1 END) * 40) DESC,
          p.created_at ASC
